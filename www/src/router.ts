...
export function populateScreen() {
  ...
  const currentManager = cardManagers.filter(
    (manager) => manager.name === globals.currentScreen
  )[0];
...
function switchScreen(buttonId: string) {
  ...
  if (oldScreen === newScreen.name) return;
